...
function addFriend() {
  ...
  if (
    isEmptyOrSpaces(nomInput.value) ||
    isEmptyOrSpaces(prenomInput.value) ||
    isEmptyOrSpaces(numeroInput.value)
  ) {
...
function texteContact() {
  for (let i = 0; i < contacts.length; i++) {
    infoContact.innerHTML += `<span class="mx-4 my-4 text-blue-800 shadow px-2 rounded font-semibold">${contacts[i].nom} ${contacts[i].prenom} ${contacts[i].numero}</span>`;
  }
}
...
function searchUser() {
  for (let i = 0; i < contacts.length; i++) {
    if (
      search.value === contacts[i].prenom ||
      search.value === contacts[i].nom ||
      search.value === contacts[i].numero
    ) {
      resultatRecherche.innerHTML = `<span class="mx-4 my-4 text-green-600 shadow-lg px-2 bg-white rounded font-semibold">${contacts[i].nom} ${contacts[i].prenom} ${contacts[i].numero}</span>`;
    }
  }
}
